...
type instance Value R (Rel phi One from c2 r)  = Ref phi from
type instance Value R (Rel phi Many from c2 r) = RefList phi from
type instance Value L (Rel phi c1  r Many to)  = RefList phi to
...
type instance InitialValues phi r (Rel phi c1 from Many to, xs) f = (InitialValues' phi r (Rel phi c1 from Many to, xs) f)
type instance InitialValues phi r  (Rel phi c1 from One to, xs) f = AppendIfTrue (TypeEq r from) (ValueWithPointer phi r L (Rel phi c1 from One to) f) (InitialValues' phi r (Rel phi c1 from One to, xs) f)


type family   InitialValues' (phi :: * -> *) r rels finalRels :: *
type instance InitialValues' phi r (Rel phi One from c1  to, xs) f = AppendIfTrue (TypeEq r to)  (ValueWithPointer phi r R (Rel phi One from c1 to) f) (InitialValues phi r xs f)

type instance InitialValues' phi r (Rel phi Many from c1 to, xs) f = InitialValues phi r xs f
...
makeEmpty :: Rel phi c1 r1 c2 r2 -> RelationStorage (Rel phi c1 r1 c2 r2)
...
data PList (phi :: * -> *) r env rels where
 ...
 PCons :: ValueWithPointer phi r dir (Rel phi c1 i1 c2 i2) rels 
       -> PList phi r env rels 
       -> PList phi r (ValueWithPointer phi r dir (Rel phi c1 i1 c2 i2) rels, env) rels
...
getValue :: (TEq phi, ERModel phi rels) 
            => Ref phi r 
            -> (Dir dir, TIndex phi (Rel phi c1 i1 c2 i2) rels) 
            -> TList RelStorage phi rels
            -> Maybe (Value dir (Rel phi c1 i1 c2 i2))
getValue ref (dir, ix) cache = f ref dir (lookupTList4 ix relations) (unRelStorage $ lookupTList ix cache)
  where f :: Ref phi r -> Dir dir -> Rel phi c1 i1 c2 i2 -> RelationStorage (Rel phi c1 i1 c2 i2) -> Maybe (Value dir (Rel phi c1 i1 c2 i2))
...
setValue
     :: (TEq phi, ERModel phi rels) => Ref phi r -> ValueWithPointer phi r dir (Rel phi c1 i1 c2 i2) rels
     -> RelCache phi rels
     -> RelCache phi rels
setValue ix1 (ix2, prf, d, tix) = modTList (RelStorage . f d ix1 ix2 (lookupTList4 tix relations) . unRelStorage) tix
 where f :: forall phi d r c1 c2 i1 i2 . TEq phi 
         => Dir d 
         -> Ref phi r 
         -> Value d (Rel phi c1 i1 c2 i2) 
         -> Rel phi c1 i1 c2 i2 
         -> RelationStorage (Rel phi c1 i1 c2 i2) 
         -> RelationStorage (Rel phi c1 i1 c2 i2)
